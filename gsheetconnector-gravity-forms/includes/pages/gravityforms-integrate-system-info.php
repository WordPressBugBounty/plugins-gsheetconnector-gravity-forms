...
<div class="system-statuswc">
   <div class="info-container">
    <h2 class="systemifo"><?php echo __("System Info", "gsheetconnector-gravityforms"); ?></h2>
      <button onclick="copySystemInfo()" class="copy"><?php echo __("Copy System Info to Clipboard", "gsheetconnector-gravityforms"); ?></button>
      <?php echo $Gravityforms_gs_tools_service->get_gfforms_system_info(); ?>
   </div>
</div>
<div class="system-Error">
    <div class="error-container">
        <h2 class="systemerror"><?php echo __("Error Log", "gsheetconnector-gravityforms"); ?></h2>
        ...
        <button onclick="copyErrorLog()" class="copy"><?php echo __("Copy Error Log to Clipboard", "gsheetconnector-gravityforms"); ?></button>
         <button class="clear-content-logs-gf"><?php echo __("Clear", "gsheetconnector-gravityforms"); ?></button>
       ...
       <input type="hidden" name="gf-ajax-nonce" id="gf-ajax-nonce" value="<?php echo wp_create_nonce('gf-ajax-nonce'); ?>" />
        ...
        <div class="copy-message" style="display: none;"><?php echo __("Copied", "gsheetconnector-gravityforms"); ?></div> <!-- Add a hidden div for the copy message -->
        <?php echo $Gravityforms_gs_tools_service->display_error_log(); ?>
    </div>
</div>
<style>
    ...
    systemInfoElements.forEach((element) => {
        if (element.innerText) {
            ...
            // Handle table data (td tags)
                ...
                let label = element.previousElementSibling ? element.previousElementSibling.innerText : ''; // Assuming label is in the previous sibling
                label = label ? label.toLowerCase().replace(/\s+/g, '_') : 'undefined'; // Format label as lowercase with underscores
