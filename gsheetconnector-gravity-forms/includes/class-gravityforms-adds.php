...
class gravityforms_gs_Connector_Adds
{
 ...
 /**
 * Display admin notice for expired or missing Google authentication.
 *
 * Checks whether the auth expired notice should be shown based on:
 * - Dismiss status
 * - Display interval timing
 * - Auth expiration flag
 *
 * Hooks the notice display into admin_notices if conditions are met.
 *
 * @return void
 */
...
/**
 * Set "remind later" interval for auth expired notice via AJAX.
 *
 * Stores current timestamp so the notice can be shown again after a delay.
...
/**
 * Permanently dismiss auth expired notice via AJAX.
 *
 * Updates option to prevent notice from showing again.
 * Uses nonce verification for security.
 *
 * @return void Sends JSON success response.
 */
...
/**
 * Render admin notice for expired Google authentication.
 *
 * Displays a warning message with actions:
 * - Connect account
 * - Remind later
 * - Dismiss permanently
 *
 * Includes nonce field for AJAX actions.
 *
 * @return void
 */
/*
