        ...
        if (!empty($forms_list)) {
            foreach ($forms_list as $key => $value) {
                if (!empty($value->title)) {
                    ...
                    // $sheet_name = !empty($value->sheet_name) ? esc_html($value->sheet_name) : __('No Sheet Name available', 'gsheetconnector-gravityforms');
                    ?>
                    <tr>
                        <td>
            ...
            ?>
            <tr>
                <td colspan="2"><?php echo __("No Gravity Forms are connected with Google Sheets.", "gsheetconnector-gravityforms"); ?></td>
            </tr>
            <?php 
        } 
        ?>
